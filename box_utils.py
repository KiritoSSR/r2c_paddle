...
def resize_image(image, desired_width=768, desired_height=384, random_pad=False):
    ...
    transform_resize = Resize(size=(round(h * scale), round(w * scale)))
...
if USE_IMAGENET_PRETRAINED:
    ...
    # For COCO pretrained
    def to_tensor_and_normalize(image):
        tensor255 = F.to_tensor(image) * 255
        return F.normalize(tensor255, mean=(102.9801, 115.9465, 122.7717), std=(1, 1, 1))